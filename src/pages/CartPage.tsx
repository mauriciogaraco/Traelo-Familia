...
export function CartPage() {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader title="Tu carrito" subtitle={`${items.length} ${items.length === 1 ? 'producto' : 'productos'}`} />
        ...
        {items.map(({ product, quantity }) => (
          <div key={product.id} className="flex gap-3 bg-surface border border-border rounded-3xl p-3">
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              ...
              <p className="text-[11px] text-text-secondary mt-0.5">{product.businessName}</p>
      ...
      <div className="px-4 mt-5">
        <div className="bg-surface border border-border rounded-3xl p-4 space-y-2.5">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-success">A coordinar</span>
          </div>
